...
async function createTestDoctor() {
    ...
    try {
        ...
        doctorToken = jwt.sign(
            { doctorId: testDoctorId, role: 'doctor', email: TEST_DOCTOR.email },
            JWT_SECRET,
            { expiresIn: '12h' }
        );
...
async function testDoctorLogin() {
    ...
    try {
        ...
        const valid = await bcrypt.compare(TEST_DOCTOR.password, doctor.password_hash);
...
async function testWorkerAccess() {
    ...
    try {
        ...
        const sampleResult = await pool.query(
            'SELECT worker_id, full_name, mobile_number, assigned_village FROM workers LIMIT 5'
        );
...
async function testDoctorAssignments() {
    ...
    try {
        ...
        if (checkTable.rows[0].exists) {
            ...
            const assignmentsResult = await pool.query(
                `SELECT COUNT(*) FROM doctor_assignments WHERE doctor_id = $1`,
                [testDoctorId]
            );
...
async function testAuditLogging() {
    ...
    try {
        ...
        const logsResult = await pool.query(
            `SELECT action, user_type, timestamp 
             FROM audit_logs 
             WHERE user_id = $1 
